...
class _ControllerCitizenAccessPageState
  ...
  Future<void> _load() async {
    ...
    // a l'ouverture).
    try {
      polls = await PollService.instance.loadPolls().timeout(
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(_lastMessage ?? 'Operation terminee.')),
      );
  ...
  Future<void> _downloadLastGeneratedQr() async {
    ...
    try {
      final bytes = await _buildQrPngBytes(
        DeepLinks.citizenAccess(createdCode.accessCode),
      );
  ...
  Widget _buildGeneratedCitizenAccessResult(ThemeData theme) {
    final createdCode = _lastResult?.accessCode;
    final rawCode = createdCode?.accessCode.trim() ?? '';

    if (createdCode == null || rawCode.isEmpty) {
      return const SizedBox.shrink();
    }

    final qrPayload = DeepLinks.citizenAccess(rawCode);
          ...
          children: [
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    'Code citoyen genere avec succes',
            ...
            Wrap(
              ...
              children: [
                ...
                OutlinedButton.icon(
                  onPressed: () => _copy(
                    qrPayload,
                    'Lien de connexion copie.',
                  ),
                  icon: const Icon(Icons.link_rounded),
                  label: const Text('Copier le lien QR'),
                ),
                OutlinedButton.icon(
                  onPressed: _downloadLastGeneratedQr,
                  icon: const Icon(Icons.download_rounded),
                  label: const Text('Telecharger le QR'),
                ),
              ],
            ),
          ],
          ...
          child: _isLoading
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      _buildGeneratedCitizenAccessResult(theme),
                      if (_lastResult?.accessCode != null)
                        const SizedBox(height: 16),
                      Card(
                        child: Padding(
                          padding: const EdgeInsets.all(24),
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              TextField(
                                ...
                                decoration: const InputDecoration(
                                  labelText:
                                      'Commentaire agent de mobilisation citoyenne optionnel',
                                ),
                              ),
                              const SizedBox(height: 16),
                              SizedBox(
