...
class _CreateControleurDialogState extends State<_CreateControleurDialog> {
  ...
  CommuneSuggestion? _findMatchingCommune(
    List<CommuneSuggestion> suggestions,
    String communeName,
  ) {
    final expected = _lookupKey(communeName);
    ...
    for (final suggestion in suggestions) {
      if (_lookupKey(suggestion.nom) == expected) {
        return suggestion;
      }
    }

    for (final suggestion in suggestions) {
      final key = _lookupKey(suggestion.nom);
      if (key.startsWith(expected) || expected.startsWith(key)) {
        return suggestion;
      }
    }
  ...
  Future<void> _resolvePostalCodeFromApi(String value) async {
    final postal = CommuneLookupService.normalizePostal(value);

    if (postal.length != 5) {
      return;
    }

    final suggestions = await CommuneLookupService.instance.search(postal);
    ...
    if (suggestions.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Aucune commune trouvee pour le code postal $postal.'),
        ),
      );
      return;
    }

    final currentCommune =
        CommuneLookupService.normalizeCommuneName(_communeCtrl.text);
    final selected = _findMatchingCommune(suggestions, currentCommune) ??
        (suggestions.length == 1 ? suggestions.first : null);

    if (selected == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Plusieurs communes correspondent au code postal $postal. Selectionnez la commune dans la liste.',
          ),
        ),
      );
      return;
    ...
    _selectCommune(selected);
  }

  Future<bool> _ensureCommunePostalConsistency() async {
    final communeName =
        CommuneLookupService.normalizeCommuneName(_communeCtrl.text);
    final postal = CommuneLookupService.normalizePostal(_postalCtrl.text);
    ...
    if (postal.length == 5) {
      ...
      if (suggestions.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Code postal $postal introuvable dans l’API Gouv.'),
          ),
        );
        return false;
      }

      final selected = _findMatchingCommune(suggestions, communeName);
      ...
      if (communeName.isEmpty && suggestions.length == 1) {
        _selectCommune(suggestions.first);
        return true;
      }

      final expected = suggestions.map((item) => item.nom).join(', ');
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            'Incoherence commune / code postal : $postal correspond a $expected, pas a "$communeName".',
          ),
        ),
      );
      return false;
    ...
    if (communeName.length >= 2) {
      final suggestions =
          await CommuneLookupService.instance.search(communeName);
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text(
          'Selectionnez une commune valide via l’autocompletion API Gouv.',
        ),
      ),
    );
    return false;
                ...
                children: [
                  Expanded(
                    child: TextFormField(
                      ...
                      onChanged: (value) {
                        final postal =
                            CommuneLookupService.normalizePostal(value);
                        if (postal.length == 5) {
                          _resolvePostalCodeFromApi(postal);
                        }
                      },
                      onFieldSubmitted: _resolvePostalCodeFromApi,
                      enabled: !_isSubmitting,
